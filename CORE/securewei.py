...
from solc_parsing.secureweiSolc import secureweiSolc
...
class securewei(secureweiSolc):

    def __init__(self, filename='message.sol', solc='solc', disable_solc_warnings=False, solc_arguments=''):
        self._detectors = []
        self._printers = []

        stdout = self._run_solc(filename, solc, disable_solc_warnings, solc_arguments)

        super(securewei, self).__init__(filename)
