...
from securewei.detectors.abstract_detector import (AbstractDetector,
                                                 DetectorClassification,
                                                 classification_txt)
from securewei.printers.abstract_printer import AbstractPrinter
from securewei.securewei import securewei

logging.basicConfig()
logger = logging.getLogger("securewei")
...
def process(filename, args, detector_classes, printer_classes):
    """
    The core high-level code for running securewei static analysis.

    Returns:
        list(result), int: Result list and number of contracts analyzed
    """
    securewei = securewei(filename)

    for detector_cls in detector_classes:
        securewei.register_detector(detector_cls)

    for printer_cls in printer_classes:
        securewei.register_printer(printer_cls)

    analyzed_contracts_count = len(securewei.contracts)

    results = []

    if printer_classes:
        securewei.run_printers()  # Currently printers does not return results

    elif detector_classes:
        detector_results = securewei.run_detectors()
...
def main():
    ...
    """
    # from securewei.detectors.examples.backdoor import Backdoor
    # from securewei.detectors.variables.uninitialized_state_variables import UninitializedStateVarsDetection
    # from securewei.detectors.attributes.constant_pragma import ConstantPragma
    # from securewei.detectors.attributes.old_solc import OldSolc
    # from securewei.detectors.attributes.locked_ether import LockedEther
    # from securewei.detectors.functions.arbitrary_send import ArbitrarySend
    # from securewei.detectors.functions.suicidal import Suicidal
    from securewei.detectors.reentrancy.reentrancy import Reentrancy
    # from securewei.detectors.variables.uninitialized_storage_variables import UninitializedStorageVars
    # from securewei.detectors.variables.unused_state_variables import UnusedStateVars
    # from securewei.detectors.statements.tx_origin import TxOrigin
    # from securewei.detectors.statements.assembly import Assembly
    # from securewei.detectors.operations.low_level_calls import LowLevelCalls
    # from securewei.detectors.naming_convention.naming_convention import NamingConvention
    ...
    from securewei.printers.summary.function import FunctionSummary
    from securewei.printers.summary.contract import ContractSummary
    from securewei.printers.inheritance.inheritance import PrinterInheritance
    from securewei.printers.functions.authorization import PrinterWrittenVariablesAndAuthorization
    from securewei.printers.summary.slithir import PrinterSlithIR
    ...
    # Handle plugins!
    for entry_point in iter_entry_points(group='securewei_analyzer.plugin', name=None):
...
def main_impl(all_detector_classes, all_printer_classes):
    ...
    for (l_name, l_level) in [('securewei', default_log),
...
def parse_args(detector_classes, printer_classes):
    parser = argparse.ArgumentParser(description='securewei',
                                     usage="securewei.py contract.sol [flag]",
