# Return the 'right' value of an expression
...
from securewei.core.expressions.assignment_operation import AssignmentOperationType
from securewei.core.expressions.expression import Expression
...
#            set_val(expression, [expression])
    ...
    def _post_member_access(self, expression):
        val = []
        if isinstance(expression.member_name, Expression):
            expr = get(expression.member_name)
            val = expr
        set_val(expression, val)
