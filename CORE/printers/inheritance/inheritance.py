...
from securewei.detectors.shadowing.shadowing_functions import ShadowingFunctionsDetection
...
class PrinterInheritance(AbstractPrinter):
    ...
    def __init__(self, securewei, logger):
        super(PrinterInheritance, self).__init__(securewei, logger)

        inheritance = [x.inheritance for x in securewei.contracts]
        self.inheritance = set([item for sublist in inheritance for item in sublist])

        shadow = ShadowingFunctionsDetection(securewei, None)
        ret = shadow.detect()
        functions_shadowed = {}
        for s in ret:
